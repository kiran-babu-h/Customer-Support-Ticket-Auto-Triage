...
def analyze_file():
    ...
    try:
        text=re.sub(r"\s+"," ",extract_uploaded_text(file)).strip()[:20000]
        if not text:
        ...
        if any(w in low for w in ["refund","payment","charged","transaction","billing"]): signals.append("payment/billing language")
        if any(w in low for w in ["urgent","immediately","asap","critical","blocked"]): signals.append("urgency language")
        if any(w in low for w in ["error","failed","cannot","can't","unable","not working"]): signals.append("failure/error language")
        if any(w in low for w in ["late","delay","not arrived","missing"]): signals.append("delivery/delay language")
        return jsonify({"filename":secure_filename(file.filename),"characters_analyzed":len(text),"category":category,"priority":priority,"department":department,"recommended_team":recommend_team(category, text),"model":META["best_model"],"priority_model":PRIORITY_MODEL_NAME,"reason":", ".join(signals[:3]) or "ticket content and learned text patterns"})
