...
def analyze_file():
    ...
    try:
        ...
        return jsonify({"filename":secure_filename(file.filename),"characters_analyzed":len(text),"category":category,"priority":priority,"department":department,"model":META["best_model"],"priority_model":PRIORITY_MODEL_NAME,"reason":", ".join(signals[:3]) or "ticket content and learned text patterns"})
